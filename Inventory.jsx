    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="flex gap-3 mb-5">
        <div className="relative flex-1 max-w-xs">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search inventory..." className="pl-8" />
        </div>
        ...
        {isLoading ? (
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-50">
              {filtered.map(v => {
                const days = v.purchase_date_raw ? Math.floor((Date.now() - new Date(v.purchase_date_raw)) / 86400000) : 0
                return (
                  <tr key={v.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => window.location.href = `/inventory/${v.id}`}>
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-xs font-medium ${days > 45 ? 'text-red-600' : days > 30 ? 'text-yellow-600' : 'text-gray-500'}`}>{days}d</span>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${statusColors[v.status] || 'bg-gray-100 text-gray-500'}`}>{v.status}</span>
                    </td>
                  </tr>
                )
              })}
