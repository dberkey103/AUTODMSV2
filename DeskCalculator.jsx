          ...
          <div className="bg-white rounded-xl border border-gray-100 p-4">
            ...
            {selectedVehicle ? (
              <div className="bg-gray-50 rounded-lg p-3 relative">
                ...
                <div className="text-sm font-semibold text-gray-900 pr-4">{selectedVehicle.year} {selectedVehicle.make} {selectedVehicle.model}</div>
                <div className="text-xs text-gray-400 mt-0.5">Stock #{selectedVehicle.stock} · ${(selectedVehicle.price || 0).toLocaleString()}</div>
              </div>
            ) : (
              <button onClick={() => setShowSearch(!showSearch)}
                className="w-full flex items-center gap-2 border border-dashed border-gray-300 rounded-lg p-3 text-sm text-gray-400 hover:border-gray-400 transition-colors">
              ...
              <div className="mt-2">
                ...
                <div className="border border-gray-100 rounded-lg divide-y divide-gray-50 max-h-40 overflow-y-auto">
                  {filteredInventory.length === 0
                    ? <div className="p-2 text-xs text-gray-400 text-center">No results</div>
                    : filteredInventory.map(v => (
                      <button key={v.id}
                        ...
                        <div className="text-xs font-medium">{v.year} {v.make} {v.model}</div>
                        <div className="text-xs text-gray-400">#{v.stock} · ${(v.price || 0).toLocaleString()}</div>
                      </button>
                    ))
                  }
                </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
            ...
            {baseSell <= 0 ? (
              <div className="p-16 text-center">
                ...
                <p className="text-sm text-gray-400">Enter a sell price to generate the payment matrix</p>
              </div>
            ) : (
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {PRICE_OFFSETS.map((offset, rowIdx) => {
                      ...
                      return (
                        <tr key={offset} className={isCenter ? 'bg-blue-50' : 'hover:bg-gray-50'}>
                          ...
                          {matrix[rowIdx].map((payment, colIdx) => (
                            <td key={TERMS[colIdx]} className={`px-3 py-3 text-center ${isCenter ? 'bg-blue-50' : ''}`}>
                              {payment !== null ? (
                                <div className={`text-sm font-semibold ${isCenter ? 'text-blue-700' : 'text-gray-800'}`}>
                                  ${payment.toFixed(0)}
                                </div>
                              ) : (
